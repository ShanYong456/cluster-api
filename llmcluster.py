...
def generate_itinerary(user_stay_days, max_hours_per_day):
    ...
    # --- Solution 1: Fit locations across requested days ---
    ...
    for day_locs in solution_days_raw:
        ...
        # Sort clusters by highest priority in each cluster
        day_clusters = []
        for cid, locs in clusters_dict.items():
            ...
            day_clusters.append({
                ...
                "locations": sorted(locs, key=lambda x: x["priority"])  # sort locations by priority
            })

        # Sort clusters by highest_priority
        day_clusters_sorted = sorted(day_clusters, key=lambda x: x["highest_priority"])
        for c in day_clusters_sorted:
            c.pop("highest_priority")  # remove helper field
